...
def make_args(**overrides) -> Namespace:
    defaults = dict(
        ...
        eval_num_gpus=0,
        partition_radix_cache_by_rollout_call=True,
    )
...
class TestKvCacheNamespace:
    def _make_eval_fn(self, monkeypatch, recorder: StampRecordingGenerate, *, partition: bool = True, **overrides):
        args = make_args(**make_eval_args(partition_radix_cache_by_rollout_call=partition), **overrides)
        fn = make_fn(monkeypatch, args, FakeDataSource())
    ...
    async def test_shared_state_eval_stays_in_the_train_namespace_the_engine_is_already_serving(self, monkeypatch):
        """Shared-engine eval reuses the training namespace, so groups started after it are not stamped eval."""
        recorder = StampRecordingGenerate()
        fn = self._make_eval_fn(monkeypatch, recorder)
        fn._curr_kv_cache_namespace = "train:-:3"

        await fn(RolloutFnEvalInput(rollout_id=4))

        assert recorder.take_stamps() == {"train:-:3"}
        assert fn._curr_kv_cache_namespace == "train:-:3"
    ...
    async def test_the_partition_being_off_names_no_namespace_for_a_train_call(self, monkeypatch):
        """With --no-partition-radix-cache-by-rollout-call the producer names no namespace at all."""
        fn = make_fn(monkeypatch, make_args(partition_radix_cache_by_rollout_call=False), FakeDataSource())
