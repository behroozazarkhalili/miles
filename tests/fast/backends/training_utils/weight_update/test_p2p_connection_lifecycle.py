...
def _peer_session_ids(protocol) -> list[str]:
    return [
        peer.session_id
        for meta in protocol._transfer_engine_meta_list
        for cell_updater in meta.cell_updaters
        for peer in cell_updater._peer_by_engine_rank.values()
    ]
...
class TestReconnection:
    """The source side of a P2P connection outlives the peers it is currently writing to."""
    ...
    def test_a_reconnection_replaces_every_peer_session(self, p2p_protocol) -> None:
        ...
        assert sorted(_peer_session_ids(protocol)) == ["b-0-0", "b-1-0"]
        assert not set(map(id, _cell_updaters(protocol))) & set(map(id, first_cell_updaters))

    def test_a_reconnection_drains_the_pending_writes_before_dropping_the_peers(self, p2p_protocol) -> None:
        """A write still in flight reads the shared buffers, so the peers must not be replaced under it."""
        protocol = _make_protocol(p2p_protocol)
        factory = _ReplicaFactory()
        observed_peers_at_drain: list[list[str]] = []
        real_drain = protocol._drain_pending_writes

        def recording_drain() -> None:
            observed_peers_at_drain.append(sorted(_peer_session_ids(protocol)))
            real_drain()
        ...
        assert observed_peers_at_drain == [["a-0-0"]]
    ...
    def test_a_write_that_never_finished_is_kept_and_does_not_block_the_reconnection(self, p2p_protocol) -> None:
        ...
        assert protocol._transfer_engine is engine_after_first
        assert protocol._shared_params_dict is params_after_first
        assert sorted(_peer_session_ids(protocol)) == ["b-0-0"]

    def test_a_rank_without_targets_drops_its_peers_and_keeps_its_buffers(self, p2p_protocol) -> None:
    ...
    def test_a_reconnection_drops_the_tensors_staged_by_an_abandoned_sync(self, p2p_protocol) -> None:
        ...
        with _patched_p2p(p2p_protocol, factory) as patches:
            _connect(protocol, patches, pairs=[(0, 0)])
            protocol._tensor_stager.get_transfer_ready_params(
                [("hf.q", torch.zeros(1))], param_mapper=mapper, params_dict={"w": torch.zeros(1)}
            )
            with pytest.raises(AssertionError):
                protocol._tensor_stager.assert_all_transferred()
...
class TestStandaloneDisconnect:
    ...
    def test_a_disconnect_drops_every_client_and_peer_reference(self, p2p_protocol) -> None:
    ...
    def test_a_disconnected_protocol_neither_registers_nor_writes(self, p2p_protocol) -> None:
        """The updater keeps driving every rank, so a disconnected one must publish nothing to the old peers."""
        protocol = _make_protocol(p2p_protocol)
        factory = _ReplicaFactory()
    ...
    def test_a_connection_after_a_standalone_disconnect_serves_the_new_peers(self, p2p_protocol) -> None:
...
class TestOneReplicaPerEngineRank:
    ...
    def test_targets_of_one_engine_rank_that_differ_only_in_placement_are_served_together(self, p2p_protocol) -> None:
        ...
        assert sorted(_peer_session_ids(protocol)) == ["a-0-0", "a-1-0"]
...
class TestInferenceCellState:
    ...
    def test_a_sender_tracks_the_cells_it_holds_no_peer_for(self, p2p_protocol) -> None:
        ...
        assert protocol._cell_updaters_by_cell_id["cell-1"]._peer_by_engine_rank == {}
    ...
    def test_a_disposed_cell_updater_submits_no_further_write(self, p2p_protocol) -> None:
        """An old peer endpoint addresses an engine that was replaced, so writing to it corrupts nothing but wastes."""
        protocol = _make_protocol(p2p_protocol)
        factory = _ReplicaFactory()
...
class TestConnectWithADeadTarget:
    ...
    def test_a_dead_engine_marks_its_cell_and_leaves_the_others_connected(self, p2p_protocol) -> None:
        ...
        assert sorted(_peer_session_ids(protocol)) == ["a-1-0"]
    ...
    def test_a_dead_engine_is_not_written_to_by_the_next_bucket(self, p2p_protocol) -> None:
        """The cell updater of a cell that never handed over its peers must submit nothing."""
        protocol = _make_protocol(p2p_protocol)
        factory = _ReplicaFactory()
        ...
        assert dead._peer_by_engine_rank == {}
...
class TestConnectWhoseEveryTargetIsDead:
    """A rank left without a single usable peer must stay out of the way, not break the trainer."""

    def test_no_usable_peer_neither_stages_nor_registers(self, p2p_protocol) -> None:
...
class TestPerCellWriteThreads:
    ...
    def test_a_write_queued_before_a_reconnection_never_reaches_the_new_cell(self, p2p_protocol) -> None:
        """The queued write still carries the peer of the replaced engine, whose memory now belongs to somebody else."""
        protocol = _make_protocol(p2p_protocol)
        factory = _ReplicaFactory()
        protocol._transfer_timeout = 0.05
        release = threading.Event()
