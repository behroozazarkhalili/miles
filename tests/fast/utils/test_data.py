...
class TestProcessRolloutData:
    ...
    @pytest.mark.parametrize("raw_factory", [_raw_batch, _singleton_batch])
    @pytest.mark.parametrize("balance_data", [False, True])
    def test_three_live_cells_partition_a_batch_none_of_them_divides(
        self, raw_factory: Callable[[], dict[str, Any]], balance_data: bool
    ):
        """256 samples over 3 cells must still land in exactly one shard each, balanced or not, splittable or not."""
        shards = _shards_of_every_rank(args=_delay_split_args(balance_data=balance_data), raw=raw_factory())
    ...
    def test_cells_hold_micro_batch_counts_within_one_of_each_other_and_different_sample_counts(self):
        """Independent cells align micro-batches to their own group, so counts may differ by one across cells."""
        shards = _shards_of_every_rank()

        counts = [sum(shard["num_microbatches"]) for shard in shards]
