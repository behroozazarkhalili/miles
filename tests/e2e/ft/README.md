...
- **Dynamic batch size**: `--use-dynamic-batch-size` is mandatory, not a tuning choice — indep_dp requires it, since only the dynamic micro-batch schedule absorbs a cell count that does not divide the global batch.
