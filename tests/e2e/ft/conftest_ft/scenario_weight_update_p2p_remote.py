...
def run_ci(mode: ModeOption, num_steps: NumStepsOption = DEFAULT_NUM_STEPS) -> None:
    ...
    print(f"Targeted p2p target fault test PASSED ({TEST_NAME}, mode={mode}, steps={num_steps})")
...
def assert_hook_outcome(run: TargetedHookRun) -> None:
    ...
    harm_observed_at = compute_victim_harm_observed_at(run.events, fire=fire, armed=armed)
