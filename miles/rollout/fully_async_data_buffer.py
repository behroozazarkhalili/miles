...
``get`` hands out a whole training batch at once: it waits until ``num_groups`` groups
pass every get-time filter and returns them together, so a batch is never half held by
a caller the checkpoint cannot see. ``snapshot`` and ``restore`` are what a checkpoint
persists and what the always-on sample ownership checker reads.
"""
...
class DataBuffer(ABC):
    ...
    ``snapshot`` reports exactly what the buffer holds and ``restore`` puts it back,
    which is what a checkpoint persists and what the sample ownership checker reads.
    A buffer that may hand out one sample several times sets ``replays_samples``, and
    the checker then only holds it to never losing a sample.
    """
    ...
    async def put(self, input: DataBufferInput) -> PutOutcomes:
        ...
        The outcome is reported per policy: a single-policy buffer keys it by ``None``,
        and a buffer composing several keys one outcome per ``trainer_model_id``.
        Returning ``None`` is invalid. Outcomes describe finished samples, whose
        ``trainer_model_id`` selects their policy. Unused prompts go to
        ``unused_handler_fn(prompt_group, reason=UnusedReason.ABORTED, trainer_model_id=None)`` on put or
        ``reason=UnusedReason.STALE`` on get.
        """
    ...
    async def get(self, *, num_groups: int, **context: Any) -> list[DataBufferInput]:
        ...
        A partial batch is never handed out: everything the caller does not get back
        stays owned by the buffer. ``context`` is the extra information for sample
        processing at get() time, including the ``trainer_model_id`` whose groups are
        asked for. Stale prompts are passed to the unused handler with the required
        keywords ``reason=UnusedReason.STALE`` and ``trainer_model_id=None`` before the batch is returned.
        """
...
class DefaultDataBuffer(DataBuffer):
    ...
    async def put(self, input: DataBufferInput) -> PutOutcomes:
        ...
        if any(s.status == Sample.Status.ABORTED for s in iter_samples(input.group)):
            ...
            return {None: PutOutcome.RECYCLED}
        ...
        if not filter_output.keep:
            ...
            return {None: PutOutcome.DROPPED}
