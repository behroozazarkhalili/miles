...
def check(events: list[Event]) -> list[WeightProgressIssue]:
    ...
    expected = adjacent_change_expected_by_publication(events)
    return [
        issue
        for key, checksums in canonical.items()
        if (issue := _check_one_publication(key=key, checksums=checksums, canonical=canonical, expected=expected))
        is not None
    ]
...
def _check_one_publication(
    *,
    ...
    if previous is None or previous != checksums:
        return None
    if not (expected[key] and expected[previous_key]):
        return None
