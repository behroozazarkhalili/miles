...
class WeightVersionsPerCall:
    ...
    def validate(self, *, num_tokens: int) -> None:
        ...
        if self.prefill_spans:
            prefill_end = _assert_spans_run_in_order(
                spans=self.prefill_spans, call=self, kind="prefill", start=0, allow_gaps=False
            )
            assert prefill_end == output_start, (
                f"prefill weight version spans of call {self} must cover exactly the {output_start} prompt tokens "
                f"of this call; the engine stamps every prompt token whose KV it computed or reused"
            )

        output_end = _assert_spans_run_in_order(
            spans=self.spans, call=self, kind="output", start=output_start, allow_gaps=True
        )
        assert (
    ...
    def from_meta_info(meta_info: dict, output_end: int) -> "WeightVersionsPerCall":
        ...
        return WeightVersionsPerCall(
            ...
            prefill_spans=_get_prefill_spans(meta_info),
            output_start=output_start,
        )


def _assert_spans_run_in_order(
    *, spans: list[WeightVersionSpan], call: WeightVersionsPerCall, kind: str, start: int, allow_gaps: bool
) -> int:
    expected_start = start
    for index, span in enumerate(spans):
        exact = index == 0 or not allow_gaps
        starts_where_expected = span.abs_start == expected_start if exact else span.abs_start >= expected_start
        assert starts_where_expected and span.abs_start < span.abs_end, (
            f"{kind} weight version span {span} of call {call} must be non-empty and start at "
            f"{'' if exact else 'or after '}token {expected_start}"
        )
        expected_start = span.abs_end
    return expected_start


def _get_prefill_spans(meta_info: dict) -> list[WeightVersionSpan]:
